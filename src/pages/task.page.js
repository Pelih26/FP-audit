...
export class CreateTask extends BasePage {
  constructor(page) {
    ...
    this.dataPicker = page.locator('div').filter({ hasText: /^Установить$/ }).nth(4);
    this.dateClick = page.locator('[data-test="Sat Oct 11 2025 00:00:00 GMT+0300 (Moscow Standard Time)"]').getByText('11', { exact: true });   // Тут надо подумать как дату менять
    this.dateChouse = page.getByRole('button', { name: 'Выбрать' });
    this.performersTab = page.locator('div').filter({ hasText: /^Исполнители$/ });
    this.storeNumberClick = page.getByRole('textbox', { name: 'Введите список номеров, разделённый запятыми или пробелами' });
    this.storeNumberFull = page.getByRole('textbox', { name: 'Введите список номеров, разделённый запятыми или пробелами' });
